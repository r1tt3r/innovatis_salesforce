...
function addProductToCart(currentBasket, productId, quantity, childProducts, options) {
    ...
    childProducts = childProducts.filter(function (product) {
        return product.quantity > 0;
    }) 

    /*product.bundledProducts = collections.reduce(product.bundledProducts, function(prod){
        return prod.ID == childProducts.map(function (productChild){
            return productChild.pid
        })
    })*/    
    ...
    var qtd_total = 0;
    childProducts.forEach(function (childProduct) {
        qtd_total = qtd_total + childProduct.quantity
    })

    if(qtd_total == 0){
        result.error = true;
        result.message = Resource.msgf(
            'error.alert.selected.quantity.cannot.be.added.for.pls.sel',
            'product',
            null,
            product.availabilityModel.inventoryRecord.ATS.value,
            product.name
    ...
    if(qtd_total < product.custom.qtd_prod){
        result.error = true;
        result.message = Resource.msgf(
            'error.alert.selected.quantity.cannot.be.added.for.not.enough',
            'product',
            null,
            product.availabilityModel.inventoryRecord.ATS.value,
            product.name
        ...
        return result;
    }else if(qtd_total > product.custom.qtd_prod){
        result.error = true;
        result.message = Resource.msgf(
            'error.alert.selected.quantity.cannot.be.added.for.too.much',
            'product',
            null,
            product.availabilityModel.inventoryRecord.ATS.value,
            product.name
